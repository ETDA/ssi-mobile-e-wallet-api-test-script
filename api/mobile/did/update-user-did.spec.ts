...
describe('Update user DID', () => {
  ...
  test('Update user DID - Send request with incorrect id', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      state.data.did = didRegister.request.data.id
      ...
      const updateDid = await UserDID.Update(state.data.did, dummyUserId, state.didKey1, state.userId)
      console.log('Update DID: ', JSON.stringify(updateDid.data, null, 2))
      expect(updateDid.status).toEqual(400)
    } catch (err) {
      ...
      expect(err.response.data.fields.id.code).toBe(INVALID.USER_ID.CODE)
      expect(err.response.data.fields.id.message).toBe(INVALID.USER_ID.MESSAGE)
    }
  })
  ...
  test('Update user DID - Send request with incorrect did_address', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      state.data.did = didRegister.request.data.id
      ...
      const updateDid = await UserDID.Update(dummyDid, state.userId, state.didKey1, state.userId)
      console.log('Update DID: ', JSON.stringify(updateDid.data, null, 2))
      expect(updateDid.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
  ...
  test('Update user DID - Send request without id', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      state.data.did = didRegister.request.data.id
      ...
      const updateDid = await UserDID.Update(state.data.did, '', state.didKey1, state.userId)
      console.log('Update DID: ', JSON.stringify(updateDid.data, null, 2))
      expect(updateDid.status).toEqual(400)
    } catch (err) {
      ...
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields.id.code).toBe(ERR_REQUIRE.USER_ID.CODE)
      expect(err.response.data.fields.id.message).toBe(ERR_REQUIRE.USER_ID.MESSAGE)
    }
  })

  test('Update user DID - Send request without did_address', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      state.data.did = didRegister.request.data.id
      ...
      const updateDid = await UserDID.Update('', state.userId, state.didKey1, state.userId)
      console.log('Update DID: ', JSON.stringify(updateDid.data, null, 2))
      expect(updateDid.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
  ...
  test('Update user DID - Send request with incorrect signature', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      state.data.did = didRegister.request.data.id
      ...
      const updateDid = await UserDID.Update(state.data.did, state.userId, state.didKey2, state.userId)
      console.log('Update DID: ', JSON.stringify(updateDid.data, null, 2))
      expect(updateDid.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
  ...
  test('Update user DID - Send request with incorrect id in URL', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      state.data.did = didRegister.request.data.id
      ...
      const dummyUserId = uuidv4()
      const updateDid = await UserDID.Update(state.data.did, state.userId, state.didKey1, dummyUserId)
      console.log('Update DID: ', JSON.stringify(updateDid.data, null, 2))
      expect(updateDid.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
