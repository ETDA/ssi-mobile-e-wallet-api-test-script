...
describe('Get mobile user by DID', () => {
  ...
  test('Get mobile user by DID', async () => {
    try {
      ...
      const getMobileUserByDid = await MobileInfo.GetMobileUserByDID(state.data.did)
      console.log('Get Mobile user by DID: ', JSON.stringify(getMobileUserByDid.data, null, 2))
      expect(getMobileUserByDid.status).toEqual(200)
      expect(getMobileUserByDid.data.id).toBe(state.userId)
      expect(getMobileUserByDid.data.first_name).toBe(state.firstName)
      expect(getMobileUserByDid.data.last_name).toBe(state.lastName)
      expect(getMobileUserByDid.data.did_address).toBe(state.data.did)
    } catch (err) {
